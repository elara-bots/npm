import { formatNumber, is, p, parser, ParserOptions, time } from "@elara-services/utils";
...
export async function giveawayParser<D>(
    obj: object,
    db: Giveaway<D>,
    ...
    return s
        ...
        .replace(new RegExp(gp.end_timer, "gi"), time.relative(new Date(db.end)))
        .replace(
            new RegExp(gp.end_format, "gi"),
            time.long.dateTime(new Date(db.end))
        )

        .replace(new RegExp(gp.start, "gi"), new Date(db.start).toISOString())
        .replace(
        ...
        .replace(
            ...
            is.array(winners) ? winners.map((c) => `<@${c}>`).join(", ") : "N/A"
        )
        .replace(
