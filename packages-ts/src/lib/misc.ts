export async function fetch<B extends object, T>(url: string, key = "", body: B | undefined = undefined, postRequest = false, returnRaw = false): Promise<T | null> {
    try {
        ...
        const res = await client.fetch(url, postRequest ? "POST" : "GET")
        .header(headers)
        .body(body, "json")
        .send()
