...
export function randomWeight<T>(objects: (T & { weight: number })[]): (T & { weight: number }) | undefined {
...
export function randomWords(options: RandomWord): string | string[] {
    const randInt = (less: number) => Math.floor(Math.random() * less);
    ...
    if (is.boolean(options.exactly)) {
        options.min = 1;
        options.max = 1;
    }

    if (!is.number(options.wordsPerString)) {
        options.wordsPerString = 1;
    }
    if (typeof options.formatter !== 'function') {
        options.formatter = (word) => word;
    }
    if (!is.string(options.separator)) {
        options.separator = ' ';
    }
    let total = 0;
    if (is.number(options.min) && is.number(options.max)) {
