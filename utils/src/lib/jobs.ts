...
export function createJob(interval: IntervalJob["interval"], run: IntervalJob["run"]): IntervalJob {
    return { interval, run };
}

export function startAllIntervalJobs<T extends Record<string, IntervalJob>>(jobs: T) {
